...
import { FsError, type FileSystem, type FsObservation, type FsTarget } from '@deepseek-ai/dsh-fs'
...
export function createDesignApplyTool(editorHost: EditorHostController, services: DesignApplyServices) {
  return defineTool({
    ...
    async execute(args: DesignApplyArgs, exec) {
      const requestedPath = args.path.trim()
      if (requestedPath.length === 0) throw new Error(`${OPENPENCIL_APPLY_TOOL_NAME}: path is required`)
      if (extname(requestedPath).toLowerCase() !== '.op') {
        throw new Error(`${OPENPENCIL_APPLY_TOOL_NAME}: path must end in .op`)
      }
      if (args.operations.trim().length === 0) throw new Error(`${OPENPENCIL_APPLY_TOOL_NAME}: operations must not be empty`)
      if (args.operations.length > MAX_OPERATIONS_LENGTH) throw new Error(`${OPENPENCIL_APPLY_TOOL_NAME}: operations are too large`)
      ...
      if (!isAbsolute(processPath)) throw new Error(`${OPENPENCIL_APPLY_TOOL_NAME}: the DSH filesystem did not provide an absolute host path`)
      const resolvedInfo = await services.fs.stat(target, exec.signal)
      if (resolvedInfo === undefined) throw new Error(`${OPENPENCIL_APPLY_TOOL_NAME}: target does not exist: ${requestedPath}`)
      if (resolvedInfo.type !== 'file') throw new Error(`${OPENPENCIL_APPLY_TOOL_NAME}: target is not a regular file: ${requestedPath}`)
      ...
      try {
        outcome = await services.fs.writeText(
          target,
          batch.documentJson,
          { kind: 'replaceIfVersion', version: baselineVersion },
          exec.signal,
          sandboxPolicy,
        )
      } catch (error) {
    ...
    presentCall: (args: DesignApplyArgs) => ({
      card: 'generic',
