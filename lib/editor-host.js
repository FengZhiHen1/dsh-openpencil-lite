...
import { EditorRecoveryStore, readManagedDaemonDocument, restoreManagedDaemonDocument, } from './editor-recovery.js';
...
function requireLoopbackPeer(req) {
...
async function waitForHandshake(child, diagnostics, signal) {
...
async function waitForEditorReady(baseUrl, signal) {
...
export class EditorHostController {
     ...
     */
    async createDocumentBatch(options) {
        const binary = this.binary;
        if (binary === undefined)
            throw new Error('OpenPencil editor host binary is unavailable');
        options.signal.throwIfAborted();
        if (this.#disposePromise !== undefined)
            throw new Error('OpenPencil editor host is shutting down');
    ...
    async #createDocumentBatch(binary, options) {
        ...
        try {
            ...
            const onAbort = () => { if (child !== undefined)
    ...
    async #launch(token, origin, res, routeGeneration, ownerSessionId) {
        ...
        try {
            ...
            try {
                ...
                let recovery;
                try {
                    recovery = await this.#recoveryStore.find(session.sourcePath, session.baselineSha256, current.toString('utf8'));
                }
                catch {
    ...
    async #save(id, req, res) {
        ...
        await this.#enqueueSessionOperation(session, async () => {
            ...
            try {
                ...
                if (session.closed)
                    throw new HttpError(410, 'editor session ended before save completed');
                let value;
                try {
    ...
    async #captureRecoveryForSession(id, reason) {
    ...
    async #captureRecovery(session, reason) {
        if (session.closed)
            return undefined;
    ...
    async #restoreRecoveryForSession(id, recoveryId) {
        const session = this.#sessionForControl(id);
        return this.#enqueueSessionOperation(session, async () => {
            session.mutating = true;
            try {
                ...
                if (recovery === undefined)
                    throw new HttpError(404, 'OpenPencil recovery snapshot not found');
                const current = await readManagedDaemonDocument(new URL(session.iframeUrl).origin, session.daemonToken);
                const version = await restoreManagedDaemonDocument(new URL(session.iframeUrl).origin, session.daemonToken, { documentJson: recovery.documentJson, version: current.version });
                session.createdAt = Date.now();
    ...
    async #discardRecoveryForSession(id, recoveryId) {
        const session = this.#sessionForControl(id);
    ...
    #enqueueSessionOperation(session, task) {
        const previous = session.activeOperation ?? Promise.resolve();
        const result = previous.then(async () => {
            if (session.closed)
                throw new HttpError(410, 'editor session ended before operation completed');
            return task();
        });
        const settled = result.then(() => { }, () => { });
        session.activeOperation = settled;
        void settled.then(() => {
            if (session.activeOperation === settled)
                delete session.activeOperation;
        });
        return result;
    }
    async #waitForSessionOperation(session) {
    ...
    async #captureThenDispose(session, reason) {
        try {
