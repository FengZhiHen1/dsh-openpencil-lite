...
import { isLoopbackRemoteAddress } from './editor-host.js';
import { MAX_DOCUMENT_BYTES, MAX_RENDER_BYTES, PRESENTATION_META_KEY, projectRenderGrant, renderDir, snapshotDir, } from './renderer.js';
...
function isSafeString(value, maxLength = 16 * 1024) {
...
function isManagedArtifactPath(path, root, filename) {
...
function parseFrame(value, expectedIndex) {
 ...
 */
export function parseHydratableRenderResult(value) {
    ...
    if (value.frames !== undefined) {
        if (!Array.isArray(value.frames) || value.frames.length === 0 || value.frames.length > MAX_HYDRATION_FRAMES)
            return undefined;
        frames = [];
        ...
        if (frames[0]?.path !== value.path
            || frames[0]?.filename !== value.filename
            || frames[0]?.bytes !== value.bytes
            || frames[0]?.sha256 !== value.sha256)
            return undefined;
    }
    if (value.frameCount !== undefined && (!isSafeInteger(value.frameCount, 1, MAX_HYDRATION_FRAMES) || value.frameCount !== frames?.length))
        return undefined;
    if (frames === undefined && value.frameCount !== undefined)
        return undefined;
    return value;
}
...
function renderResultDigest(result) {
    return createHash('sha256').update(canonicalJson(result)).digest('hex');
}
function parseHydrationRequest(value) {
    if (!isRecord(value))
        return undefined;
    const expected = new Set(['sessionId', 'callId', 'documentSha256']);
    if (Object.keys(value).length !== expected.size || !hasExactKeys(value, expected))
        return undefined;
    if (!isSafeString(value.sessionId, 256)
...
function requestAuthority(req, trustedHosts) {
    ...
    if (!loopback && !isTrustedAuthority(hostUrl, trustedHosts))
        return undefined;
    ...
    if (rawOrigin !== undefined) {
        if (typeof rawOrigin !== 'string')
            return undefined;
        try {
            if (new URL(rawOrigin).host !== hostUrl.host)
                return undefined;
        }
        catch {
            return undefined;
...
function finishEmpty(res, status) {
    setSecurityHeaders(res);
    res.writeHead(status);
    res.end();
}
function finishJson(res, value) {
...
export class PresentationHydrationController {
    ...
    constructor(dependencies, options = {}) {
        ...
        if (!Number.isSafeInteger(this.#ttlMs) || this.#ttlMs <= 0)
            throw new TypeError('presentation hydration ttlMs must be positive');
        if (!Number.isSafeInteger(this.#maxEntries) || this.#maxEntries <= 0)
            throw new TypeError('presentation hydration maxEntries must be positive');
        if (!Number.isSafeInteger(this.#maxRecordBytes) || this.#maxRecordBytes <= 0)
            throw new TypeError('presentation hydration maxRecordBytes must be positive');
        if (!Number.isSafeInteger(this.#maxBytes) || this.#maxBytes <= 0)
            throw new TypeError('presentation hydration maxBytes must be positive');
    }
    /** Observe one trusted in-process result before Code Mode drops its meta. */
    observeToolResult(exec, result) {
        if (exec.name !== OPENPENCIL_RENDER_TOOL_NAME
        ...
        if (!isSafeString(sessionId, 256) || !isSafeString(callId, 512))
            return;
        const now = this.#now();
        this.#prune(now);
    ...
    async handle(req, res) {
        try {
            ...
            if (url.pathname !== PRESENTATION_HYDRATION_ROUTE || url.search !== '')
    ...
    #resolve(request, editorAllowed) {
        ...
        const editor = authorization !== undefined
            ...
            : undefined;
        const projected = projectRenderGrant(result, this.dependencies.render, this.dependencies.viewer?.viewerGrant, editor);
        if (!isRecord(projected) || !(PRESENTATION_META_KEY in projected))
            return undefined;
        const envelope = projected[PRESENTATION_META_KEY];
        if (!isRecord(envelope))
            return undefined;
        return { [PRESENTATION_META_KEY]: envelope };
    }
    #liveAuthorization(request, result) {
        const now = this.#now();
        this.#prune(now);
        ...
        if (record.documentSha256 !== request.documentSha256
    ...
    #historicalResult(request) {
        ...
        if (settlement === undefined || settlement.duplicate || settlement.event === undefined)
            return undefined;
    ...
    #historyIndex(session) {
        const events = session.events;
        let index = this.#history.get(session);
        if (index === undefined
            ...
            this.#history.set(session, index);
        }
        ...
        for (let eventIndex = index.indexedLength; eventIndex < events.length; eventIndex += 1) {
            ...
            if (event?.type !== 'tool/code-dispatch' || event.data.name !== OPENPENCIL_RENDER_TOOL_NAME)
                continue;
            const callId = String(event.data.subCallId);
            if (!isSafeString(callId, 512))
                continue;
    ...
    #parseHistoricalEvent(event) {
        if (event.type !== 'tool/code-dispatch')
            return undefined;
        const data = event.data;
        if (data.isError !== false || !Array.isArray(data.content) || data.content.length !== 1)
            return undefined;
        const block = data.content[0];
        if (!isRecord(block) || Object.keys(block).length !== 2 || block.type !== 'text' || typeof block.text !== 'string')
            return undefined;
        if (block.text.length > MAX_HISTORY_TEXT_BYTES || Buffer.byteLength(block.text, 'utf8') > MAX_HISTORY_TEXT_BYTES)
            return undefined;
        let parsedValue;
        try {
            parsedValue = JSON.parse(block.text);
        }
        catch {
            return undefined;
        }
