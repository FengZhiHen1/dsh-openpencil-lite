...
function DesignRenderView({ block, callId, toolName, openFile, inspect, locale = "en", sessionId }) {
...
function apply(ctx) {
	...
	ctx.inject(["betterSidebar"], (injectedCtx) => {
		...
		const dispose = registerTab({
			...
			single: true,
			component: (props) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(OpenPencilPreviewTab, {
				...props,
				locale: getLocale()
			})
		});
