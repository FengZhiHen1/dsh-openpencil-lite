 ...
 */
export function createDesignNewTool(editorHost, services) {
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            if (requestedPath.length === 0)
                throw new Error(`${OPENPENCIL_NEW_TOOL_NAME}: path is required`);
            ...
            if (args.operations.trim().length === 0)
                throw new Error(`${OPENPENCIL_NEW_TOOL_NAME}: operations must not be empty`);
            if (args.operations.length > MAX_OPERATIONS_LENGTH)
                throw new Error(`${OPENPENCIL_NEW_TOOL_NAME}: operations are too large`);
            ...
            if (!isAbsolute(processPath))
                throw new Error(`${OPENPENCIL_NEW_TOOL_NAME}: the DSH filesystem did not provide an absolute host path`);
            let parentInfo;
            try {
            ...
            const outcome = await services.fs.writeText(target, batch.documentJson, { kind: 'createIfAbsent' }, exec.signal, sandboxPolicy);
