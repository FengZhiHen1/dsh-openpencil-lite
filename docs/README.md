...
- 同批确认（用户，2026-08-19）：Agent 引导文案（工具描述 + `openpencil-prototype` skill）提升为与实现同批交付的需求（R-10/AC-10）；Agent 设计闭环在 `requirements.md` 场景节改写为显式四步回路；UI 原生视觉对齐采用皮肤级重写（方向 A，R-11/AC-11，决策记录 `decisions/ui-native-skin-2026-08.md`）。
- 需求确认（用户，2026-08-20）：`.op` 文件查看器提升为正式需求（R-12/AC-12）——只读画布直读形态（客户端 `fsRead` + Web SDK 画布），原"按路径 on-demand 渲染路由"预案取消（决策记录 `decisions/op-file-viewer-canvas-2026-08.md`）；preview tab 新增"预览 PNG"蒙版按钮（R-13/AC-13，平台 `ImageLightbox`，懒加载降级）。
...
| 文档 | 唯一权威范围 |
|---|---|
| `requirements.md` | 目标、场景、功能需求、约束、非目标与验收条件 |
| `technical-details/refactor-scope.md` | 相对上游的删除/保留/新增清单、包/插件/路由/工具标识与历史回放边界 |
| `technical-details/headless-apply.md` | `openpencil_apply` 工具机制、写回与冲突语义、失败处理 |
| `technical-details/better-sidebar-preview.md` | 侧边栏预览 tab、`.op` 文件查看器与"预览 PNG"蒙版的注册、数据来源、刷新与降级语义 |
| `technical-details/deployment.md` | 发布形态、web 部署红线、迭代与验收顺序 |
